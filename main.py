...
- **必ず8〜10件以上の情報を収集するよう指示すること**
- 複数のサイトを巡回し、十分な情報を集めるまで検索を続けるよう指示すること
- **必ず実際にページを開いて内容を確認してからURLを記載すること**
- **URLは必ず browser_navigate で実際に訪問したページのURLのみを使用すること**
- ページが存在しない場合やエラーの場合は、その情報は含めないこと
...
### ニュース収集の場合
- 目的: 興味に関連する最新ニュースを8〜10件以上収集する。
- 除外基準: 公開日が古い記事は除外する。
- 出力形式の例:
  - `<URL|記事タイトル> — 公開日: YYYY/MM/DD — 概要（2〜3行）`

---

### イベント系の収集の場合
- 目的: 興味に関連する今後のイベントを8〜10件以上収集する。
- 除外基準: 開催日や応募締切がすでに過ぎているものや通年のイベントは除外する。開催日が本日から半年以上先のものは出力しない。開催日が不明なものも除外する。
- 出力形式の例:
  - `<URL|イベント名> — 開催: 日時 — 締切: 日時 — 場所/形式 — 概要（2〜3行）`

---

### 技術動向・研究発表の場合
- 目的: 興味に関連する最新の技術動向や研究成果を8〜10件以上収集する。
- 出力形式の例:
  - `<URL|タイトル> — 発表日: YYYY/MM/DD — 概要（2〜3行）`

---

### その他（汎用収集）
- 目的: ユーザーの関心に関連する最新情報を8〜10件以上収集する。
- 出力形式の例:
  - `<URL|タイトル> — 日付: YYYY/MM/DD — 概要（2〜3行）`

---

## 重要な指示
- 情報が不足している場合は、必ず追加で検索を行うよう指示すること
- 各項目の概要は2〜3行で詳しく記述するよう指示すること
- 自分でイベントやニュースを作らない。
...
上記の指示に従って、playwright/mcpを使って最新情報を収集してください。"""
